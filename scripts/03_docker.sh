...
if [ ! -f ${build_path}/atf/bl31.bin ]; then
    ...
    docker run --ulimit nofile=1024 --rm -v "${root_path}:/repo:Z" ${docker_tag} /repo/scripts/docker/build_atf.sh
fi

if [ ! -d ${build_path}/uboot ]; then
    debug_msg "Docker: Building U-Boot..."
    docker run --ulimit nofile=1024 --rm -v "${root_path}:/repo:Z" ${docker_tag} /repo/scripts/docker/build_uboot.sh
fi

if [ ! -d ${build_path}/kernel ]; then
    debug_msg "Docker: Building Kernel..."
    docker run --ulimit nofile=1024 --rm -v "${root_path}:/repo:Z" ${docker_tag} /repo/scripts/docker/build_kernel.sh
fi
...
docker run --ulimit nofile=1024 --rm -v "${root_path}:/repo:Z" ${docker_tag} /repo/scripts/docker/run_mkimage_initial.sh
...
docker run --ulimit nofile=1024 --rm --privileged --cap-add=ALL -v /dev:/dev -v "${root_path}:/repo:Z" ${docker_tag} /repo/scripts/docker/run_debootstrap.sh
